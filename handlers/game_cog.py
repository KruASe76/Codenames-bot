...
class GameCog(Cog, name="game"):
    ...
    @describe(
        member=locale_str("stats_member_param"), show=locale_str("stats_show_param")
    )
    async def stats(
        self, ctx: Context, member: Member | None = None, show: bool = False
    ) -> None:
        player = member or ctx.author
        name = (
            f"**{player.global_name if player.global_name else player.display_name}**"
        )
        ...
        if player == self.bot.user:
            ...
            if ctx.interaction:
                ...
                await ctx.interaction.response.send_message(
                    embed=game_master_embed, ephemeral=not show
                )
            else:
        ...
        if not info:
            await send_error(
                ctx, loc.errors.title, loc.errors.never_played.format(name)
            )
            return
        ...
        stats_embed.add_field(name=EMPTY, value=loc.commands.stats.note, inline=False)
        stats_embed.set_thumbnail(url=player.display_avatar)

        if ctx.interaction:
            # noinspection PyUnresolvedReferences
            await ctx.interaction.response.send_message(
                embed=stats_embed, ephemeral=not show
            )
        else:
            await ctx.reply(embed=stats_embed)

    async def start(
        self, interaction: Interaction, team1: list[User], team2: list[User]
    ) -> None:
        game_uuid = str(get_uuid())

        channel = self.bot.get_partial_messageable(
            interaction.channel_id, guild_id=interaction.guild_id
        )

        loc = await self.bot.db.localization(interaction)

        # Dictionary selection
        language_msg = await channel.send(
            embed=Embed(
                title=loc.commands.start.lang_selection_title, color=Colors.purple
            )
        )
        ...
        dict_language: str = flags_lang_rev[
            (
                await self.bot.wait_for(
                    "reaction_add",
                    check=lambda reaction, user: reaction.message == language_msg
        ...
        dict_msg_desc = map(
            ...
            range(1, 10),
            dictionaries[dict_language].values(),
        )
        dict_msg = await channel.send(
            embed=Embed(
        ...
        potential_dicts = map(
            lambda e: tuple(dictionaries[dict_language].keys())[
                REACTION_NUMBERS.index(e)
            ],
            emojis,
        )
        ...
        cap_selection_list = map(
            lambda letter, player: f"**{letter}** - {player.mention}", ALPHABET, team1
        )
        ...
        await channel.send(
            embed=Embed(
                ...
                description=loc.commands.start.cap_selected_desc.format(
                    team1_cap.mention
                ),
                color=Colors.red,
            )
        ...
        await channel.send(
            embed=Embed(title=loc.game.start_announcement, color=Colors.purple)
        )

        # Notifying everyone in direct messages
        await team1_cap.send(
            embed=Embed(
        ...
        for player in team1_pl:
            ...
            team1_pl_without.remove(
                player
            )  # Team1 player list without recipient of the message
            await player.send(
                embed=Embed(
                    title=loc.game.start_notification_title,
        ...
        for player in team2_pl:
            ...
            team2_pl_without.remove(
                player
            )  # Team2 player list without recipient of the message
            await player.send(
                embed=Embed(
                    title=loc.game.start_notification_title,
                    description=loc.game.start_notification_desc_pl.format(
        ...
        available_words = list(
            team1_words + team2_words + (endgame_word,) + no_team_words
        )  # endgame_word is single
        ...
        while game_running:
            ...
            await send_fields(
                game_uuid, channel, current_cap, other_cap, send_field_to_caps
            )
            send_field_to_caps = True

            if first_round:
                shutil.copy(Paths.cap_img(game_uuid), Paths.cap_img_init(game_uuid))
                first_round = False
            ...
            move_msg = await self.bot.wait_for(
                "message",
                check=lambda msg: msg.channel == current_cap.dm_channel
            ...
            while word_count >= 0:
                ...
                if move in no_team_words:
                    ...
                    if set(other_words) <= set(
                        opened_words
                    ):  # If all second_words are opened
                        await send_fields(game_uuid, channel, current_cap, other_cap)

                        await channel.send(
                            embed=Embed(
                                title=loc.game.game_over_title,
                                description=loc.game.game_over_desc_all.format(
                                    other_color
                                ),
                                color=Colors.red
                                if other_color == loc.game.red
                        ...
                        await self.bot.db.increase_stats(
                            current_cap.id, ("games", "games_cap")
                        )
                    ...
                    await channel.send(
                        embed=Embed(
                            title=loc.game.game_over_title,
                            description=loc.game.game_over_desc_endgame.format(
                    ...
                    if set(current_words) <= set(
                        opened_words
                    ):  # If all first_words are opened
                        await send_fields(game_uuid, channel, current_cap, other_cap)
        ...
        await channel.send(
            file=File(
                Paths.cap_img_init(game_uuid), filename="initial_captain_field.png"
            )
        )
